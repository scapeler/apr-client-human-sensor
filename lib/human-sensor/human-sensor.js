...
var ApriAppHumanSensor = ApriApps.ApriAppHumanSensor = ApriApps.ApriAppHumanSensor || ApriApps.ApriAppBase.extend(function () {
	...
	var resizeApp = function(e) {
		...
		for (var page in apriPages) {
			apriPages[page].container.style.height = (appFooterTop - apriHeaderHeight - footerOffsetHeight)+ 'px';
		}
/*
...
/*
		...
		for (var i=0;i<pages.length;i++) {
			...
			if (pages[i].getAttribute("pageName") == _nextPage ) {
				...
				if (apriPages[_nextPage] && apriPages[_nextPage].pause ) apriPages[_nextPage].pause();  // close or deactivate page
			}
		}
		
		if (apriPages[_nextPage] && apriPages[_nextPage].activate ) apriPages[_nextPage].activate();  // initialize or activate page
*/
...
//						- apri-view-field-inq
	...
	var initApriPages	= function() {
		...
		apriPages.locationPage.init 		= function (container) {
			...
			locationMap = L.map(_displayElements.locationPageMapElement).setView([appConfig.mapCenterLat, appConfig.mapCenterLng], appConfig.viewZoomLevel);
