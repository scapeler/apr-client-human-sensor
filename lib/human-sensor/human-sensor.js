...
*/
	...
	var activateAction	= function(eventType, messageData) {
		...
		if (eventType == "humansensordata") {
			if (twm.activeFaseId == undefined) {
				if (twm.fase[_messageData.id]) {
					twm.activeFaseId	= _messageData.id;
					twm.fase[_messageData.id](_messageData);
				}
			} else {
				twm.fase[twm.activeFaseId].close();
...
//		loadModule('socketio', activateSocketIO);
	...
	this.initView = function() {
		...
		this.getTemplate(_template, {urlSystemRoot: apriConfig.urlSystemRoot, appPath: 'client/', app: 'human-sensor' }, {}, function(result) {
			...
			displayElements.notification 			= appBodyContainer.getElementsByClassName("notification")[0];
...
if (_page.geoLocationMap.tap) map.tap.disable();
...
if (_page.geoResultMap.tap) map.tap.disable();
