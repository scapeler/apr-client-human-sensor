...
//		}
	...
	var activateSocketIO	= function() {
		...
		var socket = io.connect('http://localhost:3010');   //this works
//		var socket = io.connect('http://149.210.208.157:3010');   //this works
//		var socket = io.connect('http://openiod.org'); //this works (after opening port on socket server for proxy server)
//		var socket = io.connect('https://openiod.org'); //this works (after opening port on socket server for proxy server)
	...
	var pushMessageHandler	= function(eventType, data) {
		...
		if (eventType == "humansensordata") {
			if (displayElements.notification) {
				...
				displayElements.notification.innerHTML	= messageData.text;
				setTimeout(function() {
...
//		};
	...
	this.initView = function() {
		...
		this.getTemplate(_template, {urlSystemRoot: apriConfig.urlSystemRoot, appPath: 'client/', app: 'human-sensor' }, {}, function(result) {
			...
			displayElements.progressBar 			= appBodyContainer.getElementsByClassName("progressbar")[0];
