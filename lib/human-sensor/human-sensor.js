...
var ApriAppHumanSensor = ApriApps.ApriAppHumanSensor = ApriApps.ApriAppHumanSensor || ApriApps.ApriAppBase.extend(function () {
	...
	var parsedUrl;
	var geoLocation, geoLocationPrev;
	...
	var activateAction	= function(eventType, messageData) {
		...
		if (eventType == "humansensordata") {
			if (twm.activeFaseId == undefined) {						// no active fase at this moment
				if (twm.fase[_messageData.actionType]) {				// is fase function available?
					twm.activeFaseId		= _messageData.id;
					twm.activeActionType	= _messageData.actionType;
					twm.fase[_messageData.actionType](_messageData);	// activate fase function
				}
			} else {
				twm.fase[twm.activeActionType].close();  // close previous action
				twm.activeFaseId			= undefined;
				twm.activeActionType		= undefined;
				if (twm.fase[_messageData.actionType]) {
					twm.activeFaseId		= _messageData.id;
					twm.activeActionType	= _messageData.actionType;
					twm.fase[_messageData.actionType](_messageData);
				}
			}
		}
	}
	...
	this.initView = function() {
		...
		this.getTemplate(_template, {urlSystemRoot: apriConfig.urlSystemRoot, appPath: 'client/', app: 'human-sensor' }, {}, function(result) {
			...
			displayElements.nrofconnections.addEventListener('click', function(e) {
				addNotification({text:'Dit geeft het aantal actieve gebruikers aan.'});
			});
...
*/
		...
		apriPages.locationPage.gotoLocation	= function (map) {
			...
			if (geoLocationPrev.lat != geoLocation.lat || geoLocationPrev.lng != geoLocation.lng) {
...
*/
		...
		apriPages.resultPage.resetScore 	= function () {
			var _page = apriPages.resultPage;
			
			var dataProperties	= {};
			if (geoLocation.neighborhoodCode) {
				for (var i=0;i<_page.data.length;i++) {
					if (_page.data[i].properties.bu_code == geoLocation.neighborhoodCode) {
...
var twmInit	= function() {
	...
	twm.fase.A001	= function(m) {
		var currTime 	= new Date().getTime();
		var timeToGo	= m.endTime - currTime;
		timeToGo		= timeToGo>0?timeToGo:0;
		console.log('timeToGo: '+ timeToGo);
		
		addNotification({text:m.text});
		
		if (displayElements.notification) {
			var innerHTMLText = '';
			displayElements.notification.classList.add('notification-enabled');
			...
			if (geoLocation.automatic == false) {
				...
				innerHTMLText += error;
				displayElements.notification.classList.add('notification-error');
				playSound(); // beep
			}
			displayElements.notification.innerHTML	= innerHTMLText;
			setTimeout(function() {
				twm.fase.A001.close();
			},timeToGo);
		}	
		twmProgressBarInit(timeToGo);
		//goToPage('locationPage');

	}
	twm.fase.A001.close	= function() {
		if (twm.activeFaseId=='A001') {
			twm.activeFaseId	= undefined;
			displayElements.notification.classList.remove('notification-enabled');
			displayElements.notification.classList.remove('notification-error');
			displayElements.notification.innerHTML	= '';

			//displayElements.twmProgressBar.parentElement.classList.remove('enabled');
		}	
	};

	twm.fase.A002	= function(m) {
		var currTime 	= new Date().getTime();
		var timeToGo	= m.endTime - currTime;
		timeToGo		= timeToGo>0?timeToGo:0;
		console.log('timeToGo: '+ timeToGo);
		
		addNotification({text:m.text, type:'gold'});

		if (displayElements.notification) {
			displayElements.notification.classList.add('notification-enabled');
			displayElements.notification.innerHTML	= m.text;
			setTimeout(function() {
				twm.fase.A002.close();
			},timeToGo);
		}	
	...
	var twmProgressBarInit	= function(timeToGo) {
		...
		twmProgressBarIncrease(); // first time before delays with setInterval
		twmProgressBarIntervalId	= setInterval(twmProgressBarIncrease, 25);

	}
	var twmProgressBarIncrease	= function() {
		var twmProgressBarPerc;
		twmPrograssBarTimeProcessed += 25;
		...
		if (twmProgressBarPerc==100) twmProgressBarclose();
	}

	var twmProgressBarclose	= function() {
