...
//		initGraphicTrend(graphOptions);
	...
	var nextPage = function () {
		...
		for (var i=0;i<pages.length;i++) {
			...
			if (pages[i].getAttribute("pageName") == this.getAttribute("nextPage") ) {
				if (this.getAttribute("nextPage")=='succes-page') {
					sendData({latlng:{lat:0,lng:0}});
				}
	...
	var sendData = function(data) {
		ajaxGetData(siteUrl + "/data/aireas/setBuurtRookRisicoMarker/*?lat=" + data.latlng.lat + "&lng=" + data.latlng.lng,
			function(data) {
