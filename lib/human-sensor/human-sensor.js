...
*/
	var pushMessageHandler	= function(eventType, data) {
        ...
        console.log('Push message from server '+eventType + ": " + data);
		if (eventType == "humansensordata") {
			if (twm.activeFaseId == undefined) {
				if (twm.fase[messageData.id]) {
					twm.activeFaseId	= messageData.id;
					twm.fase[messageData.id](messageData);
				}
			} else {
...
//		]
		...
		geoLocation.lat				= 51.45401;
		geoLocation.lng				= 5.47668;
		geoLocation.automatic		= false;
...
//		socketUrl	= '';  // default for production env
	...
	twm.fase.A001	= function(m) {
		...
		if (displayElements.notification) {
			...
			if (geoLocation.automatic == false) {
				var error = '<BR/><BR/>Uw locatie en de buurt is nog niet automatisch vastgesteld met behulp van geolocatie.<BR/>Zet dit aan om mee te kunnen doen aan de vragenlijst. ';
				innerHTMLText += error;
