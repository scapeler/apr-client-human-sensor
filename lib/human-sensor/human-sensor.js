...
var ApriAppHumanSensor = ApriApps.ApriAppHumanSensor = ApriApps.ApriAppHumanSensor || ApriApps.ApriAppBase.extend(function () {
	...
	var introText,locationText,surveyText,succesText,resultText,aboutText,statisticText;	
...
//		socket.connect();
		...
		socket.on( 'aireassignal', function (data) {
			...
			var messageText	= _signalData.signalDateTimeStr + ': ' + _signalData.message +'<BR/>Nieuwe waarde: ' + _signalData.scaqi +'. ' + '<BR/>' + 'Buurt: '+ _signalData.bu_naam+ ' (inw:'+_signalData.aant_inw_buurt+')<BR/>' + 'Wijk: '+ _signalData.wk_naam+'<BR/>'+'Gemeente: '+ _signalData.gm_naam;
...
//		socketPath	= apriConfig.urlSystemRoot + '/socket.io';
		...
		if (cookieLocationHistoryInput==undefined) {
			...
			var newGeoLocation 			= cookieLocationHistory[0];
			newGeoLocation.manual 		= false;
			newGeoLocation.automatic 	= false;
...
//			  		, {id:'escape', label:'nvt'		, color:'grey', className:'answer-escape', score:0, status:'on' }
		...
		locationText 	= '<H2>In welke buurt bevindt u zich?</H2>U kunt hier uw locatie aangeven.<BR/>Locatie bepaling gaat indien mogelijk via GPS en dus automatisch.<BR/>Locatie wordt vertaald naar de buurt waarin u zich begeeft.<BR/><BR/>'   
//		locationText 	= '<H2>In welke buurt bevindt u zich?</H2>U kunt hier uw locatie aangeven.<BR/>Indien de locatie op de kaart niet helemaal juist is, kunt u de marker verplaatsen naar de buurt waarvan u uw beleving wilt aangeven.<BR/><BR/>'   
		...
		resultText 		= '<H2>Hoe doet uw buurt het?</H2>Dit zijn de resultaten van de afgelopen periode. Het kan even duren voordat uw reactie verwerkt is en hier in beeld komt. Gebruik de "Vernieuw data" knop om nieuwe data op te halen.<BR/></BR>'; 
		...
		statisticText 	= '<H2>Project AiREAS Statistieken</H2><BR/><BR/>';
...
//		}
		...
		if (geoLocationAutomatic && geoLocation.lat==geoLocationAutomatic.lat&&geoLocation.lng==geoLocationAutomatic.lng) {
...
//			displayElements.progressBar.style.width = (pe.loaded/pe.total)*100+'%';
	...
	var dispatchEvent = function(eventType) {
		// the old fashioned way because IE edge (25-5-2016)
		//	var event = new Event('resize'); // this is the correct manner
		var event = document.createEvent('Event');
		event.initEvent(eventType,true,true);
		window.dispatchEvent(event); //force resize. Especially for leaflet maps
	}

	var login 	= function() {

		var apriAjax = new ApriAjaxDataView({
			url:'http://localhost:3000/SCAPE604/login'
...
//			}
	...
	var sendButtonPushed = function () {
		...
		if (responseAvailable) {
			...
			// save latest location from where observation sent
			var _newLocation 				= {};
			...
			_newLocation.automatic			= geoLocation.automatic;
			_newLocation.manual				= geoLocation.manual;
...
//			&region=EHV		&lat=50.1		&lng=4.0		&category=airquality		&value=1
	...
	var geoFindMe = function(page) {
		...
		if (geoLocationAutomatic) {
			debugConsole('geoLocationAutomatic: '+ geoLocationAutomatic.lat + '/' + geoLocationAutomatic.lng);
		} else {
			debugConsole('geoLocationAutomatic: '+ 'not ready ' + geoLocationAutomatic);
		}	
		...
		if (geoLocationAutomatic == undefined) {
			...
			debugConsole('Het automatisch bepalen van uw locatie is op dit apparaat niet mogelijk.');
			geoLocation.automatic	= false;
			return false;
		}
		if (geoLocationAutomatic == {}) {
			...
			debugConsole('Het automatisch bepalen van uw locatie is op dit apparaat uitgezet.');
			geoLocation.automatic	= false;
			return false;
		}
		
		if (geoLocation) {
			if ((geoLocation.lat==geoLocationAutomatic.lat&&geoLocation.lng==geoLocationAutomatic.lng) && geoLocationMarker ) {
				geoLocation.automatic	= true;
			} else {
				setGeoLocation({lat:geoLocationAutomatic.lat, lng:geoLocationAutomatic.lng})
				debugConsole('Het automatisch bepalen van uw locatie is op een andere locatie uitgekomen.');
				console.log('Het automatisch bepalen van uw locatie is op een andere locatie uitgekomen.');
...
//	};
	...
	var geoLocationAutomaticLoop	= function() {
		console.log('start geoLocationAutomaticLoop');
		if (!navigator.geolocation){
			geoLocationAutomatic 	= undefined;
			console.log('geoLocationAutomaticLoop not available');
			return false;
		}
		if (document[hidden]) {
			debugConsole('document is hidden ' + geoLocation.manual);
			console.log('geoLocationAutomaticLoop hidden');
			return;
		}
		if (geoLocation.manual != true ) {
			console.log('geoLocationAutomaticLoop manual ' +geoLocation.manual );
			debugConsole('geolocation.manual is not true ' + geoLocation.manual);
			navigator.geolocation.getCurrentPosition(geoLocationAutomaticSuccess, geoLocationAutomaticError,{maximumAge:0, timeout:10000, enableHighAccuracy:true});
		} else {
			console.log('geoLocationAutomaticLoop manual ' +geoLocation.manual );
			debugConsole('geolocation.manual is true' + geoLocation.manual);
		}		
	}
	...
	var geoLocationAutomaticSuccess	= function(position) {
		...
		geoLocationAutomatic 		= {};
		geoLocationAutomatic.lat	= Math.round(position.coords.latitude*10000000)/10000000;
		geoLocationAutomatic.lng	= Math.round(position.coords.longitude*10000000)/10000000;
		console.log(geoLocationAutomatic.lat + ' ' + geoLocationAutomatic.lng);
	};
...
//		http://openiod.com/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=geoLocationArea&lat=51.45401&lng=5.47668&format=xml
	...
	var getCbsBuurtNearestAirboxes = function() {
//		http://openiod.com/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=geoLocationArea&lat=51.45401&lng=5.47668&format=xml


		var _url = openiodUrl + '/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=nearestAirboxes&format=xml';
		_url = _url + '&region=EHV' + '&bu_code=' + geoLocation.neighborhoodCode ;
		ajaxGetData(_url,
			function(data) {
				
				var result = JSON.parse(data);
				
				geoLocation.nearestAirboxes		= result;
...
*/
		...
		apriPages.locationPage.activate 	= function () {
			...
			_page.geoLocationAutomaticLoopInterval = setInterval(geoLocationAutomaticLoop, 5000);  // reset geolocationAutomatic every 5 sec
			...
			if (_page.container == undefined) {
				...
				_page.init(_page.container, function() {
					...
					_page.timerGeoLocation = setInterval(function() {geoFindMe(_page); }, 5000);  // reset geolocation every 1 sec
					//window.dispatchEvent(new Event('resize')); //force resize. Especially for leaflet maps
					dispatchEvent('resize');
		...
		apriPages.locationPage.deactivate 	= function () {
			...
			clearInterval(_page.geoLocationAutomaticLoopInterval);
...
*/			
		...
		apriPages.aboutPage.linkedPage	= {};
		apriPages.aboutPage.linkedPage['statisticPage']	= {};
		...
		apriPages.aboutPage.init 		= function (container) {
			...
			_displayElements.pageContentElement = document.createElement('div');
			//var introPageBodyBodyTextElement 	= introPage.getElementsByClassName("apri-view-body-body-text")[0];
			_displayElements.pageContentElement.innerHTML = 'test';
			_displayElements.pageBodyElement.appendChild(_displayElements.pageContentElement);
			_displayElements.pageContentElement.addEventListener('click', login, false);
		...
		//=== Page statisticPage functions =========
		_pageName = 'statisticPage';
		createPage(_pageName);
		
		apriPages.statisticPage.linkedPage	= {};
		apriPages.statisticPage.linkedPage['aboutPage']	= {};		
			
		apriPages.statisticPage.init 		= function (container, callBack) {
		
			var _init_page = apriPages.statisticPage.init_page.bind(this, container, callBack);
			var self = this;
			if (!window.ApriD3GraphicTrend) {
				loadModule('apri-client-human-sensor-d3', _init_page
				);
			} else apriPages.statisticPage.init_page(container, callBack);

		}
		
		apriPages.statisticPage.init_page 		= function (container, callBack) {

			var _page 				= apriPages.statisticPage;
			if (_page.displayElements == undefined) _page.displayElements = {};
			var _displayElements 	= _page.displayElements;
			_page.active 			= false;			
			...
			var textElement = document.createElement('div');
			textElement.innerHTML = statisticText;
			_displayElements.pageContentElement.appendChild(textElement);
			_displayElements.graphicContainerElement = document.createElement('div');
			_displayElements.graphicContainerElement.className	= 'statistic-page-graphic-container';
			_displayElements.pageContentElement.appendChild(_displayElements.graphicContainerElement);
		...
		apriPages.statisticPage.activate 	= function () {
			var _page = apriPages.statisticPage;
			if (_page.active == true) return;
			
			if (_page.container == undefined) {
				_page.container	= appBodyContainer.getElementsByClassName("statisticPage")[0];
				_page.init(_page.container, function() {
		
					_page.getGraphicData();
					setPageVisible(_page);
		...
		apriPages.statisticPage.getGraphicData 	= function (options) {

			var _page = apriPages.statisticPage;
			if (_page.apriAjaxBase == undefined) _page.apriAjaxBase	= new ApriCore.ApriAjaxBase();
			
			var _options = {}; //options;
			
			_options.url = "https://openiod.org/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=observation_aireas_average";
			
			_options.container 		= _page.displayElements.graphicContainerElement;
			_options.description	= 'Maandgemiddelden fijnstof PM10 voor airbox 37';
			
			
			_options.groupBy 		= 'airbox';

			_options.histYear		= 'all';
			_options.histMonth		= 'all';
			_options.histDay		= '';
			_options.histYearFrom	= '2014';
			_options.histYearTo		= '2016';
			_options.histMonthFrom	= '';
			_options.histMonthTo	= '';
			
			_options.airbox			= '37.cal';
			_options.areas			= 'EHV';
			_options.sensor			= 'PM10';
			_options.reportType		= 'N';
			_options.graphOrigin	= 'year';
			_options.airboxTD		= 'N';
			
			_options.parameters		= '';			
			_options.parameters 	+= "&area=" 				+ _options.area;
			_options.parameters 	+= "&airbox=" 				+ _options.airbox;
			_options.parameters 	+= "&featureofinterest=" 	+ _options.airbox;
			_options.parameters		+= "&avgType=" 				+ _options.sensor;
			_options.parameters		+= "&sensor=" 				+ _options.sensor;
			_options.parameters		+= "&type=" 				+ _options.reportType;
			_options.parameters		+= "&interpolate=" 			+ _options.interpolate;
			_options.parameters		+= "&histYear=" 			+ _options.histYear; 
			_options.parameters		+= "&histMonth=" 			+ _options.histMonth;
			_options.parameters		+= "&histDay=" 				+ _options.histDay;
			_options.parameters		+= "&histYearFrom=" 		+ _options.histYearFrom; 
			_options.parameters		+= "&histYearTo=" 			+ _options.histYearTo; 
			_options.parameters		+= "&histMonthFrom=" 		+ _options.histMonthFrom;
			_options.parameters		+= "&histMonthTo=" 			+ _options.histMonthTo;
			_options.parameters		+= "&graphOrigin=" 			+ _options.graphOrigin;	
			_options.parameters		+= "&airboxCumulate=" 		+ _options.airboxTD;	
			
			_options.parameters		+= "&format=json";	
			
			_page.apriAjaxBase.request(_options.url + _options.parameters, {}, _options, _page.createGraphicTrend, null );
		}	

		apriPages.statisticPage.createGraphicTrend	= function (result, options) {
			console.log('CreateGraphicTrend');
			var i,j, key, 
				inpRecord, outRecord, outRecord2,
				data=[];
			...
			if (jsonResponse[0] && jsonResponse[0].hist_year ) {
				for(i=0;i<jsonResponse.length;i++) {
					...
					if (options.groupBy == 'airbox') {
						...
						_dateRelative = new Date(inpRecord.hist_year-_prefixYearDelta, (inpRecord.hist_q*3)-2-1, 1);  //first day of quarter
					}
...
//					if ( inpRecord.airbox == '12.cal' ) continue;
			...
			options.width	= options.container.clientWidth;
		...
		apriPages.statisticPage.pause 		= function () {
		}
		
		apriPages.statisticPage.deactivate 	= function () {
			var _page = apriPages.statisticPage;
			setPageHidden(_page);
		}

		apriPages.statisticPage.close 		= function () {
		}
		
		//=== Page statisticPage functions End =========
...
var debugConsole = function(text) {

	return;
	
	if (apriPages.aboutPage && apriPages.aboutPage.displayElements && apriPages.aboutPage.displayElements.pageContentElement) {
		var debugLine = text+'<BR/>';
		var textElement = document.createElement('div');
		textElement.innerHTML = debugLine;
		apriPages.aboutPage.displayElements.pageContentElement.appendChild(textElement);
	}	
}	
