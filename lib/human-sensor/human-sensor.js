...
var ApriAppHumanSensor = ApriApps.ApriAppHumanSensor = ApriApps.ApriAppHumanSensor || ApriApps.ApriAppBase.extend(function () {
	...
	window.getDevicePixelRatio = function () {
    	...
    	if (window.screen.systemXDPI !== undefined && window.screen.logicalXDPI       !== undefined && window.screen.systemXDPI > window.screen.logicalXDPI) {
        	...
        	ratio = Math.round((window.screen.systemXDPI / window.screen.logicalXDPI)*100)/100;
    	}
...
//	}
	...
	this.constructor = function(objectId, options) {
		...
		introText = '<H3>Hoe beleeft u uw leefomgeving?</H3><img style="float:right;width:13rem;" src="'+apriConfig.urlSystemRoot+'/client/apri-client-human-sensor/images/scapeler-od-idee.png"></img> <BR><BR><BR>'; 
...
//			element.innerHTML = "<p>Geolocation is not supported by your browser</p>";
		...
		function success(position) {
			...
			var img = new Image();
			img.src = "https://maps.googleapis.com/maps/api/staticmap?center=" + geoLocation.lat + "," + geoLocation.lng + "&zoom=15&size=400x400&sensor=true";
			img.style.width = '100%';
