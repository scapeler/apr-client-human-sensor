...
//		source.addEventListener('remove', removeHandler, false);
	...
	var pushMessageHandler	= function(e) {
        console.log('Push message from server '+e.type + ": " + (e.type === "message" ? e.data : source.url));
	};
...
//		};
	...
	this.initView = function() {
		...
		this.getTemplate(_template, {urlSystemRoot: apriConfig.urlSystemRoot, appPath: 'client/', app: 'human-sensor' }, {}, function(result) {
			...
			appBodyContainer.appendChild(progressBar);
...
//		initGraphicTrend(graphOptions);
	...
	function ajaxGetData(url, callback ) {
		...
		xmlhttp.onloadstart = function(pe) {
			...
			progressBar.value = 0;
			progressBar.style.width = pe.loaded/50+'px';
		}
		...
		xmlhttp.onloadend = function(pe) {
			progressBar.value = pe.loaded;
			progressBar.style.width = 0; //pe.loaded/50+'px';
		}
		xmlhttp.send();
	}
