...
//	}
	...
	this.constructor = function(objectId, options) {
		...
		succesText = '<H3>Bedankt voor uw deelname</H3>Uw antwoorden op de vragen over de beleving van de leefomgeving zijn belangrijk voor het onderzoek. Wij nodigen u vooral uit om deze vragen vaker te beantwoorden, zeker als daar aanleiding toe is. <BR><BR>Ga door na de volgende pagina om de resultaten van de afgelopen periode te kunnen bekijken.<BR><BR><BR>'; 
...
//			apriMenuFooterFixedElement.style.top = (appFooterTop) + 'px';
			...
			if (nextPagePage) {
				...
				nextPageElement.innerHTML = 'Ga door';
			...
			var locationPageMapElement = document.createElement('div');
			locationPageMapElement.style.height='100%';
/*
			locationMap 	= apriLeafLetBase.createMap(locationPageMapElement, {'topAreaHeight':85, viewCoordinates: [appConfig.mapCenterLat, appConfig.mapCenterLng], viewZoomLevel: appConfig.viewZoomLevel
			});
*/
...
/*
			locationMapLayers.openBasisKaartLayer = L.tileLayer.wms("http://www.openbasiskaart.nl/mapcache?SRS=EPSG%3A28992", {  
				layers: 'osm',
				format: 'image/png',
				transparent: true,
				tiled: true,
			...
			locationMapLayers.openBasisKaartLayer.addTo(locationMap);
*/

			locationMap = L.map(locationPageMapElement).setView([appConfig.mapCenterLat, appConfig.mapCenterLng], appConfig.viewZoomLevel);
...
*/
			...
			displayElements.locationPageBodyElement.appendChild(locationPageMapElement);
...
//			}).addTo(locationMap);
			...
			if (nextPagePage) {
				var nextPageElement = document.createElement('div');
				nextPageElement.innerHTML = 'Verzend uw keuzes';
				nextPageElement.setAttribute('nextpage',nextPagePage);
			...
			//var introPageBodyBodyTextElement 	= introPage.getElementsByClassName("apri-view-body-body-text")[0];
			succesPageContentElement.innerHTML = succesText;
			...
			if (nextPagePage) {
				var nextPageElement = document.createElement('div');
				nextPageElement.innerHTML = 'Bekijk de resultaten';
				nextPageElement.setAttribute('nextpage',nextPagePage);
			...
			//var introPageBodyBodyTextElement 	= introPage.getElementsByClassName("apri-view-body-body-text")[0];
			resultPageContentElement.innerHTML = resultText;
			...
			if (nextPagePage) {
				var nextPageElement = document.createElement('div');
				nextPageElement.innerHTML = 'Terug naar de startpagina';
				nextPageElement.setAttribute('nextpage',nextPagePage);
...
//		initGraphicTrend(graphOptions);
	...
	var nextPage = function () {
		...
		for (var i=0;i<pages.length;i++) {
			...
			if (_nextPage == 'survey-page') {
				if (geoLocation.lat == undefined) {
					_nextPage = 'result-page';
				};
			};
			if (pages[i].getAttribute("pageName") == _nextPage ) {
				if (_nextPage=='succes-page') {
					sendData({latlng:{lat:geoLocation.lat,lng:geoLocation.lng}, category:'airquality', value:'2'});
				}
		...
		window.dispatchEvent(new Event('resize')); //force resize for leaflet maps
