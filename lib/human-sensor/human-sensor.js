...
//	}
	...
	this.constructor = function(objectId, options) {
		...
		categories			= [
			  {category: { id:"airquality", label:"Lucht", question:"Hoe ervaart u nu de luchtkwaliteit?"
			  	, answers: [
...
//			  		, {id:'escape', label:'nvt'		, color:'grey', className:'answer-escape', score:0, status:'on' }
			...
			, {category: { id:"noicestress", label:"Geluid", question:"Heeft u nu last van omgevingsgeluiden?"
			  	, answers: [
...
//			  		, {id:'escape', label:'nvt'		, color:'grey', className:'answer-escape', score:0, status:'on' }
			...
			, {category: { id:"trafficstress", label:"Verkeer", question:"Heeft u nu last van verkeer in uw omgeving?"
			  	, answers: [
					  {id:'01', label:'Nee'			, color:'green', className:'answer-1of3', score:1, status:'off' }
			  		, {id:'02', label:'Neutraal'	, color:'yellow', className:'answer-2of3', score:2, status:'off' }
			  		, {id:'03', label:'Ja'			, color:'red', className:'answer-3of3', score:3, status:'off' }
//			  		, {id:'escape', label:'nvt'		, color:'grey', className:'answer-escape', score:0, status:'on' }
			...
			, {category: { id:"stress", label:"Stress", question:"Ervaart u nu stress?"
			  	, answers: [
					  {id:'01', label:'Nee'			, color:'green', className:'answer-1of3', score:1, status:'off' }
			  		, {id:'02', label:'Neutraal'	, color:'yellow', className:'answer-2of3', score:2, status:'off' }
			  		, {id:'03', label:'Ja'			, color:'red', className:'answer-3of3', score:3, status:'off' }
...
*/
		...
		introText 		= '<H2>Vliegveld Eindhoven sluit, AiREAS onderzoekt effect op de mens</H2><img src="'+apriConfig.urlSystemRoot+'/client/apri-client-human-sensor/images/vliegtuig.jpg"></img><BR/>Het vliegveld van Eindhoven gaat twee weken dicht. Twee weken lang niet vliegen, geen verkeer van en naar de luchthaven, alleen 400 vrachtwagens die de start en landingsbaan moeten vernieuwen met asfalt. Dat is een eenmalige kans voor samenwerkingsverband “gezonde gebiedsontwikkeling” AiREAS om eens te kijken wat het effect is van zo´n vliegveld op de mens en omgeving door vóór, tijdens en na de sluiting dat te onderzoeken op gebied van luchtverontreiniging, geluid, stress en gezondheid. De mogelijke unieke contrasten kunnen ons helpen een beeld te scheppen van wat er nodig zou zijn om middels de innovatiekracht van onze Brainport regio te komen tot een zo´n goed mogelijke mobiliteit­oplossing binnen de kaders van een zo gezond mogelijke en leefbare leefomgeving.<BR/><a href="https://aireas.wordpress.com/2016/04/28/vliegveld-eindhoven-sluit-aireas-onderzoekt-effect-op-de-mens/" target="_blank">Meer informatie</a><BR/><BR/>'; 

		locationText 	= '<H2>In welke buurt bevindt u zich?</H2>U kunt hier uw locatie aangeven. Indien de locatie op de kaart niet helemaal juist is, kunt u de marker verplaatsen naar de buurt waarvan u uw beleving wilt aangeven.<BR/><BR/>'   
		...
		resultText 		= '<H2>Hoe doet uw buurt het?</H2>TEST! Dit zijn de resultaten van de afgelopen 24 uur. Het kan even duren voordat uw reactie verwerkt is en hier in beeld komt. Gebruik de "Vernieuw data" knop om nieuwe data op te halen.<BR/>'; 


		aboutText 		= '<H2>Project AiREAS Airport</H2>Het vliegveld van Eindhoven gaat twee weken dicht. Twee weken lang niet vliegen, geen verkeer van en naar de luchthaven, alleen 400 vrachtwagens die de start en landingsbaan moeten vernieuwen met asfalt. Dat is een eenmalige kans voor samenwerkingsverband “gezonde gebiedsontwikkeling” AiREAS om eens te kijken wat het effect is van zo´n vliegveld op de mens en omgeving door vóór, tijdens en na de sluiting dat te onderzoeken op gebied van luchtverontreiniging, geluid, stress en gezondheid. De mogelijke unieke contrasten kunnen ons helpen een beeld te scheppen van wat er nodig zou zijn om middels de innovatiekracht van onze Brainport regio te komen tot een zo´n goed mogelijke mobiliteit­oplossing binnen de kaders van een zo gezond mogelijke en leefbare leefomgeving.<BR/><BR/>';
...
//						marker.setLatLng([position],{id:uni,draggable:'true'}).bindPopup(position).update();
						...
						getNeighborhood(map, geoLocation);
					});
...
//			element.innerHTML = "Unable to retrieve your location";
		...
		if (geoLocation.manual != true || geoLocation.lat == undefined) {
			navigator.geolocation.getCurrentPosition(success, error);
		}		
	};

	var getNeighborhood = function(map, geoLocation) {
//		http://openiod.com/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=geoLocationArea&lat=51.45401&lng=5.47668&format=xml
		...
		ajaxGetData(_url,
			function(data) {
				...
				apriPages.locationPage.displayElements.locationPageNeighborhoodContentElement.innerHTML =  apriPages.locationPage.bu_naam;
				apriPages.locationPage.displayElements.locationPageCityContentElement.innerHTML 		=  apriPages.locationPage.gm_naam;
...
//var _url= "http://openiod.com/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=areas&format=xml"
		...
		ajaxGetData(_url,
			function(data) {
				...
				apriPages.resultPage.cbsBuurtProjectEhvAirportLayer.clearLayers();
				apriPages.resultPage.cbsBuurtProjectEhvAirportLayer.addData(result);
...
//				}
		...
		apriPages.locationPage.init_page 		= function (container, callBack) {
			...
			_displayElements.locationPageLocationDescriptionElement = document.createElement('div');
			...
			_displayElements.locationPageNeighborhoodContentElement.className 		= 'location-neighborhood-content';
			...
			_displayElements.locationPageCityContentElement.className 		= 'location-city-content';
			...
			_displayElements.locationPageMapElement.style.height='18rem';
			_displayElements.locationPageMapElement.style.width='18rem';
			...
			_page.geoLocationMap = L.map(_displayElements.locationPageMapElement, {'attributionControl': false})
				.setView([appConfig.mapCenterLat, appConfig.mapCenterLng], appConfig.viewZoomLevel);
		...
		apriPages.locationPage.activate 	= function () {
			...
			if (_page.container == undefined) {
				...
				_page.init(_page.container, function() {
					...
					_page.timerGeoLocation = setInterval(function() {geoFindMe(_page); }, 4000);  // reset geolocation every 4 sec
...
//				}
		...
		apriPages.resultPage.init_page 		= function (container, callBack) {
			...
			_page.changeCategory	= function(e) {
				...
				apriPages.resultPage.cbsBuurtProjectEhvAirportLayer.setStyle();
				apriPages.resultPage.activate();
			};
			...
			_displayElements.pageContentElement = document.createElement('div');
			_displayElements.pageContentElement.innerHTML = 'Kies hier de vraag en periode voor resultaat ';
			_displayElements.pageBodyElement.appendChild(_displayElements.pageContentElement);
			...
			for (var i=0; i<categories.length;i++) {
				...
				_displayElements.pageSelectCategoryOptionElement.innerHTML = _category.label;
			...
			_displayElements.pageSelectCategoryOptionElement.innerHTML = 'Laatste 24 uur';
			_displayElements.pageSelectCategoryElement.appendChild(_displayElements.pageSelectCategoryOptionElement);
//			_displayElements.pageSelectCategoryOptionElement = document.createElement('option');
...
//			_displayElements.pageSelectCategoryElement.appendChild(_displayElements.pageSelectCategoryOptionElement);
			...
			_page.geoResultMap = L.map(_displayElements.resultPageMapElement, {'attributionControl': false})
				.setView([_page.mapCenterLat, _page.mapCenterLng], _page.viewZoomLevel);
...
/*
			  ...
			  {category: { id:"airquality", label:"Luchtkwaliteit", question:"Hoe ervaart u de luchtkwaliteit?"
			  	, answers: [
					  {id:'01', label:'Goed'		, className:'answer-1of3', score:1, status:'off' }
			  		, {id:'02', label:'Neutraal'	, className:'answer-2of3', score:2, status:'off' }
			  		, {id:'03', label:'Slecht'		, className:'answer-3of3', score:3, status:'off' }
			  		, {id:'escape', label:'nvt'		, className:'answer-escape', score:0, status:'on' }
					]
				}}
...
*/
   	     			...
   	     			return {color: '#000000', opacity:0.5, fillColor:_color, weight:1, fillOpacity:_fillOpacity };
   	 			}
				, onEachFeature:onEachcbsBuurtProjectEhvAirportFeature
